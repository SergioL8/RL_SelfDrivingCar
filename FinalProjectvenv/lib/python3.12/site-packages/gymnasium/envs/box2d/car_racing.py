...
SCALE = 6.0  # Track scale = 6.0
TRACK_RAD = 700 / SCALE  # Track is heavily morphed circle with this radius = #900
...
TRACK_TURN_RATE = 1 # = 0.31
TRACK_WIDTH = 45 / SCALE # = 40
BORDER = 8 / SCALE
BORDER_MIN_COUNT = 4
...
class CarRacing(gym.Env, EzPickle):
    ...
    * `continuous=True` converts the environment to use discrete action space.
    ...
    def draw_point(self, x_map, y_map, color, radius):
        x_cam, y_cam = self.world_to_coord(x_map, y_map)
        pygame.draw.circle(self.surf, color, (int(x_cam), int(y_cam)), radius)


    def compute_lasers(self, observation):
        x_car, y_car = self.car.hull.position
        i = int(0)
        # res = []
        # y_coor = y_car
        # while(y_coor < 96 or y_coor > 0):
        #     y_coor = y_car + i
        #     i +=1
        #     res.append((x_car, y_coor))
        # return res


    def draw_lasers(self, color, rays):
        for (dist, sx, sy, ex, ey) in rays:
            screen_start = self.world_to_coord(sx, sy)
            screen_end = self.world_to_coord(ex, ey)
            pygame.draw.line(self.surf, (0, 0, 255), screen_start, screen_end, 2)



    def compute_forward_rays(self, num_points=20, step=1):
        """
        Computes a list of points along the car's forward direction.
        
        Parameters:
            num_points: How many points to compute (i.e. the ray length in steps).
            step: The distance between consecutive points (in world units).
        
        Returns:
            A list of tuples (x, y) in world coordinates.
        """
        # Get the car's world position and heading angle.
        x_car, y_car = self.car.hull.position
        theta = self.car.hull.angle + math.pi/2  # in radians

        rays = []  # initialize the list of ray points
        for i in range(0, num_points * step, step):
            # Compute the new point along the forward direction:
            ray_x = x_car + i * math.cos(theta)
            ray_y = y_car + i * math.sin(theta)
            pixel_array = pygame.surfarray.array3d(self.screen)
            pixel_array = np.transpose(pixel_array, (1, 0, 2))
            screen_coord = self.world_to_coord(ray_x, ray_y, 'window')
            col = int(screen_coord[0])
            row = int(screen_coord[1])
            pixel_color = pixel_array[row, col]
            if ((pixel_color == (34, 139, 34)).all()):
    ...
    def draw_rays(self, rays, color=(0, 255, 0), radius=5):
        """
        Draw each ray point on the current surface.
        
        Parameters:
            rays: List of (x, y) points in world coordinates.
            color: The color to draw the points.
            radius: The radius (in pixels) of the drawn circle.
        """
        # for (x, y) in rays:
        #     self.draw_point(x, y, color, radius)
        if rays:
            last_ray = rays[-1]
            self.draw_point(last_ray[0], last_ray[1], color, radius)
    ...
    def _init_colors(self):
        if self.domain_randomize:
            ...
            self.road_color = np.array([200, 200, 200])
            self.bg_color = np.array([34, 139, 34])
            self.grass_color = np.array([34, 139, 34])
        else:
            # default colours
            self.road_color = np.array([200, 200, 200])
            self.bg_color = np.array([34, 139, 34])
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        self.rays = self.compute_forward_rays(20, 1)
    ...
    def _render(self, mode: str):
        ...
        x_car, y_car = self.car.hull.position
        self.draw_point(x_car, y_car, (0, 0, 255), 3)
        self.draw_rays(self.rays, (0, 0, 255))
        # INSERT DRAWINGS HERE
